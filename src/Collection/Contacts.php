...
class Contacts implements JsonSerializable,  ArrayAccess, SeekableIterator, Countable
{
    /** @var DateHelper */
    ...
    public function createFromJson($json = null)
    {
        ...
        if (is_array($json)) {
            if (isset($json["contacts"]) && is_array($json["contacts"])) {
                foreach ($json["contacts"] as $field) {
                    if (isset($field["contactId"])) {
                        $contact = new Contact($this->dateHelper);
                        $contact->createFromJson($field);
                        $this->add($contact);
                    }
                }
            }
