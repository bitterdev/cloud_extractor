...
class Locations implements JsonSerializable,  ArrayAccess, SeekableIterator, Countable
{
    /** @var DateHelper */
    ...
    public function createFromJson($json = null)
    {
         ...
         *
         * @link project://docs/icloud/services/friends/get-friends.md
         * @link project://docs/icloud/services/friends/locate-friend.md
         */

        if (isset($json["content"]) && is_array($json["content"])) {
            foreach ($json["content"] as $field) {
                if (isset($field["location"]) && is_array($field["location"])) {
                    $location = new Location($this->dateHelper);
                    $location->createFromJson($field);
                    $this->add($location);
                }
            }
        }
