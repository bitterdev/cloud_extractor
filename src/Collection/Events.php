...
class Events implements JsonSerializable,  ArrayAccess, SeekableIterator, Countable
{
    /** @var DateHelper */
    ...
    public function createFromJson($json = null)
    {
        ...
        if (is_array($json)) {
            if (isset($json["Event"]) && is_array($json["Event"])) {
                foreach ($json["Event"] as $field) {
                    if (isset($field["startDate"])) {
                        $event = new Event($this->dateHelper);
                        $event->createFromJson($field);
                        $this->add($event);
                    }
                }
            }
